...
const SHUTTING_DOWN_ERR_MSG = 'Shutting down experiment runner'
...
module.exports = class Runner extends ReadyResource {
  constructor (experiments, resBee, logger, { testInterval, tempStorageDir } = {}) {
    ...
    this.logger = logger
    this.tmpStorageDir = tempStorageDir || '/tmp/temp-scale-test-storage' // TODO: expose as config, and use test-tmp in tests
    this.resBee = resBee
  ...
  async _close () {
    ...
    this._forceShutdown(new Error(SHUTTING_DOWN_ERR_MSG))
  ...
  async _clearStorage () {
    try {
      await fsProm.rm(this.tmpStorageDir, { recursive: true, force: true })
    } catch (e) {
  ...
  async _runExperiment () { // Should never throw
    ...
    try {
      ...
      if (e.message === SHUTTING_DOWN_ERR_MSG) {
        this.logger.info(`Canceling experiment '${info}' because the runner is shutting down`)
      } else {
        this.logger.error(`${info} failed`)
        this.logger.error(e)
      }
      this.lastRunTime = null
      ...
      // Ignored if the timeout promise is already in rejected state
